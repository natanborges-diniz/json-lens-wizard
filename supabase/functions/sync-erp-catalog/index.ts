...
serve(async (req) => {
  ...
  try {
    ...
    prices.forEach((p: Record<string, unknown>, i: number) => { if (p.erp_code) codeIdx.set(String(p.erp_code), i); });
    ...
    for (const raw of erpRows) {
      ...
      if (pi != null) {
        ...
        if (row.esferico_min != null || row.esferico_max != null) av.sphere = { min: row.esferico_min ?? price.availability?.sphere?.min, max: row.esferico_max ?? price.availability?.sphere?.max };
        if (row.cilindrico_min != null || row.cilindrico_max != null) av.cylinder = { min: row.cilindrico_min ?? price.availability?.cylinder?.min, max: row.cilindrico_max ?? price.availability?.cylinder?.max };
        if (row.adicao_min != null || row.adicao_max != null) av.addition = { min: row.adicao_min ?? price.availability?.addition?.min, max: row.adicao_max ?? price.availability?.addition?.max };
        if (row.diametro_min != null || row.diametro_max != null) { av.diameter_min = row.diametro_min ?? price.availability?.diameter_min; av.diameter_max = row.diametro_max ?? price.availability?.diameter_max; }
        ...
        updated++;
        if (sampUpd.length < 5) sampUpd.push({ erp_code: ec, description: row.descricao, availability: price.availability, active: price.active, blocked: price.blocked });
      } else if (createMissing) {
        const fid = resolveFamily(row.descricao, dict, engine, familyIdSet, supplier);
        if (fid) {
          const np = { erp_code: ec, description: row.descricao, family_id: fid, index: extractIdx(row.descricao, idxRegex), active: row.ativo ?? true, blocked: row.bloqueado ?? false, price_sale_half_pair: row.preco || 0, availability: { sphere: { min: row.esferico_min ?? 0, max: row.esferico_max ?? 0 }, cylinder: { min: row.cilindrico_min ?? 0, max: row.cilindrico_max ?? 0 }, addition: { min: row.adicao_min ?? 0, max: row.adicao_max ?? 0 }, diameter_min: row.diametro_min ?? 0, diameter_max: row.diametro_max ?? 0 } };
          prices.push(np);
          created++;
    ...
    if (applyFlag && !dryRun) {
      const { count: ep } = await supabase.from('catalog_pending_skus').select('*', { count: 'exact', head: true }).eq('supplier_code', supplier).eq('status', 'pending');
      ...
      const supFamIds = families.filter((f: Record<string, unknown>) => (f.supplier as string)?.toUpperCase() === supplier && f.active).map((f: Record<string, unknown>) => f.id);
      const noAv = prices.filter((p: Record<string, unknown>) => supFamIds.includes(p.family_id) && p.active && !p.blocked && (!p.availability || (!(p.availability as Record<string, unknown>).sphere && !(p.availability as Record<string, unknown>).cylinder)));
      ...
      if (gates.some(g => g.status === 'block')) {
        const { data: run } = await supabase.from('catalog_sync_runs').insert({ supplier_code: supplier, run_type: 'apply_blocked', status: 'blocked', rows_read: rowsRead, rows_matched: matched, rows_updated: updated, rows_created: created, rows_not_found: notFound.length, pending_skus_count: pendingToCreate.length, file_name: fileName, report: { gates } }).select('id').single();
        const rid = run?.id;
        ...
        return new Response(JSON.stringify({ mode: 'erp-sync', supplier, dry_run: false, applied: false, blocked: true, gates, sync_run_id: rid, rows_read: rowsRead, rows_ignored: rowsIgnored, matched, updated, created, not_found_in_catalog: notFound.length, pending_created: pendingToCreate.length, missing_family_mapping: missingMap.slice(0, 20), supplier_mismatch_conflicts: conflicts.slice(0, 10), sample_updates: sampUpd, sample_created: sampCre }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }
    }

    // Record run
    const { data: run } = await supabase.from('catalog_sync_runs').insert({ supplier_code: supplier, run_type: dryRun ? 'dry_run' : 'apply', status: dryRun ? 'completed' : 'pending', rows_read: rowsRead, rows_matched: matched, rows_updated: updated, rows_created: created, rows_not_found: notFound.length, pending_skus_count: pendingToCreate.length, file_name: fileName, report: { gates, sample_updates: sampUpd } }).select('id').single();
    ...
    if (!dryRun && applyFlag) {
      ...
      const { error: upErr } = await supabase.storage.from('catalogs').upload('catalog-default.json', new Blob([JSON.stringify(catalog)], { type: 'application/json' }), { upsert: true, cacheControl: '0' });
      if (upErr) {
        if (runId) await supabase.from('catalog_sync_runs').update({ status: 'failed' }).eq('id', runId);
        return new Response(JSON.stringify({ error: 'Upload failed', details: upErr.message }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }
      ...
      await supabase.from('catalog_versions').insert({ version_number: 'erp-sync-' + supplier.toLowerCase() + '-' + new Date().toISOString().split('T')[0], schema_version: catalog.schema_version || '1.2', import_mode: 'erp-sync', dataset_name: 'ERP Sync - ' + supplier, families_count: af, prices_count: ap, changes_summary: { supplier, matched, updated, created, not_found: notFound.length, gates }, notes: ['ERP sync ' + supplier + ': ' + updated + ' updated, ' + created + ' created'] });
      if (runId) await supabase.from('catalog_sync_runs').update({ status: 'completed' }).eq('id', runId);
      applied = true;
    }

    return new Response(JSON.stringify({ mode: 'erp-sync', supplier, dry_run: dryRun, applied, blocked: false, gates, sync_run_id: runId, rows_read: rowsRead, rows_ignored: rowsIgnored, matched, updated, created, not_found_in_catalog: notFound.length, not_found_codes: notFound.slice(0, 20), pending_created: pendingToCreate.length, missing_family_mapping: missingMap.slice(0, 20), supplier_mismatch_conflicts: conflicts.slice(0, 10), sample_updates: sampUpd, sample_created: sampCre }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  } catch (error) {
